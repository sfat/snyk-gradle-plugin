...
function getVersionBuildInfo(gradleVersionOutput: string): VersionBuildInfo {
  const gradleOutputArray = gradleVersionOutput.split('\n');

  // from first 3 new lines, we get the gradle version
  const gradleVersion = gradleOutputArray[2].split(' ')[1].trim();
  ...
  versionMetaInformation.map((value) => value.replace('/(\r\n|\n|\r)/gm', ''))
  .filter((value) => value && value.length > 0)
  .map((value) => value.split(/:(.+)/))
  .forEach((splitValue) => metaBuildVersion[toCamelCase(splitValue[0].trim())] = splitValue[1].trim());
